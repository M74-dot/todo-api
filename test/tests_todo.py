...
#         MagicMock(todo_id=1, title='Reading Book', status='Not Done', user=mock_user_model)
#     ]
...
def test_post_todo_success():
    with app.app_context():
        with patch("models.todo.TodoModel") as mock_todo_model:
            ...
            # Mock the db.session.add() method to do nothing.
            with patch("app.db.session.add") as mock_add:
                # Mock the db.session.commit() method to do nothing.
                with patch("app.db.session.commit") as mock_commit:
                    # Call the post() method with the expected parameters.
                    response = TodoList.post(TodoList(), todo_data={"title": "Test Todo", "status": "Incomplete"}, user_id=1)

                    # Assert that the response contains the expected TodoModel properties.
                    assert response == {"todo_id": 1, "title": "Test Todo", "status": "Incomplete", "user": 1}

                    # Assert that db.session.add() was called with the expected TodoModel instance.
                    mock_add.assert_called_once_with(mock_todo)

                    # Assert that db.session.commit() was called once.
